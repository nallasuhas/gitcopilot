# GitGuard — Repository Analysis Report



> A thorough, code-grounded analysis of the `gitguard-main` repository.
...
rewrite shared history. GitGuard lets a user state a git goal in plain English
...
| Natural-language → git command agent loop | `gitguard/agent.py` (`Agent.run`) |

| 3-tier permission classifier (read-only / mutating / destructive) | `gitguard/classifier.py` (`classify`) |

| Flag-aware classification (same subcommand, different tiers) | `gitguard/classifier.py` (`_classify_branch`, `_classify_push`, etc.) |

| Plan-then-apply blast-radius previews (read-only) | `gitguard/preview.py` (`build_preview`) |

| Repo-state guard (detached HEAD, conflicts, ahead/behind, dirty) | `gitguard/repostate.py` (`snapshot`) |

| Structured tool-calling (no prose parsing) | `gitguard/planner/base.py` (`ToolCall`, `TOOL_SCHEMA`) |

| Offline deterministic planner (CI/demos, no model) | `gitguard/planner/offline.py` (`OfflinePlanner`) |

| Ollama-backed planner (local LLM, JSON-schema constrained) | `gitguard/planner/ollama.py` (`OllamaPlanner`) |

| JSON-lines transcript + `replay` | `gitguard/transcript.py` (`Transcript`) |

| Shell-free execution, timeouts, no-pager/no-prompt | `gitguard/gitcmd.py` (`GitRunner`) |

| Explain mode (propose + classify, run nothing) | `gitguard/agent.py` (`AgentConfig.explain_only`) + `gitguard/cli.py` (`ExplainPrinter`) |
...
├── .gitignore                  # ignores .venv, .gitguard/, scratch-repo/, build artifacts
...
├── gitguard/                 # the Python package

│   ├── __init__.py             # exports classify, Classification, Tier, __version__

│   ├── __main__.py             # `python -m gitguard` entry → cli.main
...
There are two equivalent entry points, both delegating to `gitguard.cli.main`:



1. **Console script** (declared in `pyproject.toml` line 30):

   `gitguard = "gitguard.cli:main"` — installed by `pip install -e .`, provides

   the `gitguard` command.



2. **Module execution** (`gitguard/__main__.py`):
...
1. The shell invokes `gitguard [flags] "<goal>"` (or `python -m gitguard ...`).

2. `gitguard/cli.py:main` parses args with `argparse` (line 204-215).
...
- **No runtime frameworks.** The Ollama client is hand-rolled on
  ...
  `urllib.request` (`gitguard/planner/ollama.py:81-102`); ANSI output is

  hand-rolled (`gitguard/cli.py:38-58`). This is an explicit design choice
...
- **setuptools** (`build-backend = "setuptools.build_meta"`,
  ...
  `[tool.setuptools.packages.find]` with `include = ["gitguard*"]`.
...
- **git** — the real `git` binary on `PATH`, invoked via `subprocess.run` with
  ...
  `shell=False` (`gitguard/gitcmd.py:68`).
...
### `gitguard/classifier.py` — The Safety Core
...
### `gitguard/agent.py` — The Loop & Permission Gate
...
### `gitguard/gitcmd.py` — Safe Git Execution
...
### `gitguard/repostate.py` — Repo-State Guard
...
### `gitguard/preview.py` — Plan-Then-Apply Previews
...
### `gitguard/planner/base.py` — The Tool-Call Contract
...
### `gitguard/planner/offline.py` — Deterministic Backend
...
### `gitguard/planner/ollama.py` — LLM Backend
...
### `gitguard/transcript.py` — Audit Log
...
### `gitguard/cli.py` — Terminal UI & Wiring
...
Trace of a typical run: `gitguard --offline --repo scratch-repo "what changed in my working tree?"`
...
2. **Transcript init** (line 220): `Transcript(".gitguard/log.jsonl")`.
...
For `gitguard --offline --explain --repo scratch-repo "undo my last commit but keep the changes"`:
...
```
            ...
            │  Transcript.write(TranscriptRecord)  -> .gitguard/log.jsonl │
...
- **`.gitguard/log.jsonl`** — append-only JSON-lines transcript
...
- **Transcript file** — written at end of run; readable via `gitguard replay`.
...
| `GITGUARD_MODEL` | `ollama.py:49` | `llama3.2` | Ollama model name |
...
| `--transcript F` | `.gitguard/log.jsonl` | JSON-lines log path |
...
- `[tool.setuptools.packages.find]`: `include = ["gitguard*"]`.
...
- **Steps**:
  ...
  6. **Smoke test**: `./scripts/make_scratch_repo.sh` then two `gitguard`
...
this possible). The smoke test additionally exercises the installed `gitguard`
...
2. **`gitguard/classifier.py`** — the safety core; the flag-aware tier rules

   are the heart of the project. Read `classify` + `_classify_branch` +

   `_classify_push` + `_classify_clean` first.

3. **`gitguard/planner/base.py`** — the `ToolCall` contract and `Planner`

   Protocol; understand the two-action structured-calling model before the

   planners.

4. **`gitguard/agent.py`** — the loop (`Agent.run`) and the gate

   (`Agent._gate`); how the classifier's tiers become runtime decisions.

5. **`gitguard/preview.py`** — how destructive commands get blast-radius

   previews from read-only queries.

6. **`gitguard/repostate.py`** — the surprise-state guard.

7. **`gitguard/gitcmd.py`** — the safe execution wrapper (short but critical).

8. **`gitguard/planner/offline.py`** — the deterministic backend; shows the

   intent→command pattern and makes the loop runnable without a model.

9. **`gitguard/planner/ollama.py`** — the real LLM backend; the system prompt

   and JSON-schema constraint.

10. **`gitguard/cli.py`** — the wiring (argparse, ANSI, confirmers, transcript).

11. **`gitguard/transcript.py`** — the audit log (small).
...
2. **`gitguard/classifier.py`** — read `Tier`, `Classification`, then `classify`
...
3. **`gitguard/planner/base.py`** — `ToolCall`, `Planner` Protocol,

   `TOOL_SCHEMA`. Understand the two-action contract.

4. **`gitguard/agent.py`** — `Agent.run` (the loop) and `Agent._gate` (the

   gate). See how tiers become decisions.

5. **`gitguard/preview.py`** — `build_preview` + 2-3 builders

   (`_preview_reset`, `_preview_clean`, `_preview_push`).

6. **`gitguard/repostate.py`** — `snapshot` + `_detect_in_progress`.

7. **`gitguard/gitcmd.py`** — short; note the `shell=False`, timeout, and

  no-pager/no-prompt env injection.

8. **`gitguard/planner/offline.py`** — `_plan_for` (the intent matcher).

9. **`gitguard/planner/ollama.py`** — the system prompt + `_chat` + `next_action`.

10. **`gitguard/cli.py`** — `main` (wiring), `interactive_confirmer`,

    `Printer`, `_record`.

11. **`gitguard/transcript.py`** — small; the JSON-lines format.
...
importable and creates the `gitguard` console script. For a distributable
...
- Inspect `.gitguard/log.jsonl` after a run, or `gitguard replay` to
...
Removes `.venv`, `scratch-repo`, `.gitguard`, egg-info, `__pycache__`.
